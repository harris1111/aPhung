...
module.exports.run = async function({ api, event }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "BOT lê chính" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		return api.sendMessage(`⚡️lê chính bot  Connected Successfully!⚡️\n\n⚡️Cảm ơn bạn đã sử dụng con bot của tuii nhaa⚡️\n\n⚡️Lưu ý đầu tiên khi dùng bot là hãy chat "luật bot" để đọc luật sử dụng BOT. K đọc ăn ban sml ráng chịu bot đc support by đặng thị kim linh điều hành by Lê Chính ⚡️\n\n⚡️Để tránh bot spam level và thông báo gỡ tin dẫn đến lag thì hãy dùng /resend và /rankup nhé ⚡️\n\n⚡️Chúc bạn sử dụng vui vẻ UwU <3`, threadID);
	}
	else {
		try {
